...
Measured spawn, movement, and interpreter benchmark status is maintained in `bench/runtime.md`.

## Current runtime benchmark conclusions

| Topic | Durable conclusion | Canonical evidence |
| --- | --- | --- |
| Movement | Keep `utils.move_to()` as the generic helper. Use known-current-coordinate specialization only where an algorithm already owns authoritative coordinates. | `bench/runtime.md`, `move-v2` |
| Spawn topology | Keep balanced binary as the generic topology; optimize worker locality before micro-optimizing binary versus powers-of-two fan-out. | `bench/runtime.md`, `spawn-v5` |
| Domain-specific fan-out | Use hard-coded Flekay fan-out only when an end-to-end domain benchmark shows a material win. | `bench/runtime.md`, `spawn-v5` |
